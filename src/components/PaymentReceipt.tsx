...
const BOX_BG = { r: 240, g: 250, b: 251 }; // Light teal background to match new color scheme
...
async function fetchImageAsDataURL(url: string): Promise<string> {
  // Handle local files from public folder (paths starting with /)
  if (url.startsWith('/')) {
    // For local files, use the full URL with current origin
...
export async function generatePaymentReceiptPDF({
  ...
  try {
    ...
    // Draw company name first to ensure it displays clearly
    ...
    const maxLogoWidth = 85; // Adjusted for proper header fit
    const maxLogoHeight = 45; // Adjusted for header alignment
...
itemsLayout.forEach(itemObj => {
  ...
  doc.text(wrappedText, textX, lineY);
    ...
    return {
      dataUrl: pdfDataUrl,
      blob: pdfBlob,
      fileName: `Payment_Receipt_${referenceNumber}_${receivedFrom.replace(/\s+/g, '_')}.pdf`
    };
  } catch (error) {
...
// Function to directly download (for backward compatibility)
export async function downloadPaymentReceiptPDF(props: PaymentReceiptProps) {
  const result = await generatePaymentReceiptPDF(props);
